...
from food import food_create, CALORIES
...
def skewer_calories(skewer):
    """
    finds calories present on the skewer
    :param skewer:
    :return:
    """
    if skewer.top is None:
...
def skewer_vegan(skewer):
    """
    finds if skewer is vegan friendly or not
    :param skewer: skewer
    :return: string which says whether skewer is vegan friendly or not
    """
    if skewer.top is None:
        return "Skewer has nothing on it!"
    else:
